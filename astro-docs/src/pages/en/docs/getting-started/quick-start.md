...
ETLBox is developed in C# and targets .NET Standard 2.0 and higher. It can be used by almost every [.NET version](EXT:https://docs.microsoft.com/en-us/dotnet/standard/net-standard#net-implementation-support) out there supported by Microsoft, including .NET Framework 4.X, .NET Core 2.X, .NET Core 3.X, .NET 5.0 and .NET 6.0. If you are still using .NET Framework, we recommend to use a Framework version not less than 4.7.2.

All ETLBox [packages are hosted on nuget](EXT:https://www.nuget.org/packages?q=etlbox). You will need to install always the [core package](EXT:https://www.nuget.org/packages/ETLBox/) when working with ETLBox. It  contains the default connectors and transformations. 
...
The connectors are in separate packages - depending on your needs, choose the right connector package from the list. Pick the [ETLBox.SqlServer](EXT:https://www.nuget.org/packages/ETLBox.SqlServer/) package if you want to connect to a (Azure) SQLServer database, or choose [ETLBox.Json](EXT:https://www.nuget.org/packages/ETLBox.Json/) if you want to load data from a Json file or REST endpoint. 
...
Both components support the [logging API](EXT:https://docs.microsoft.com/en-us/dotnet/core/extensions/logging) that works with a variety of built-in and third party logging providers. 
...
- Internet access to retrieve order data from [www.etlbox.net/demo/api/orders](EXT:https://www.etlbox.net/demo/api/orders)

#### Preparation

Let's start with creating the destination table on the databases. If you have Docker installed, it is quite easy to run a [SQLServer](EXT:https://hub.docker.com/_/microsoft-mssql-server) in a container (you can also other any other supported database, e.g. [MySql](EXT:https://hub.docker.com/_/mysql) ).
...
:::note
If you have a different database already installed, simply replace the connection string and connection manager with the database type you are using. For example, if you have a MySql database, you can use the MySqlConnectionManager.
:::
...
[The whole code for this example is available on GitHub](EXT:https://github.com/etlbox/etlbox.demo/tree/main/QuickStart).
...
First step is to add a dependency to the latest nuget package [NLog.Extensions.Logging](EXT:https://www.nuget.org/packages/NLog.Extensions.Logging).
...
Now you are ready to try out ETLBox. All [ETLBox packages are available on nuget](EXT:https://www.nuget.org/packages?q=etlbox). The free versions allows you to process up to 5.000 for a component - this should be enough to perform tests in your test environment. If you are ready to deploy ETLBox in your production environment, [please purchase the adequate license](/buy/pricing/). If you want to try ETLBox without limitations, you can [request a free trial key](/support/contact/request-trial/).
