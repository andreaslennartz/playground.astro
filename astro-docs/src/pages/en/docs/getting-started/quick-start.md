...
All ETLBox [packages are hosted on nuget](https://www.nuget.org/packages?q=etlbox). You will need to install always the [core package](https://www.nuget.org/packages/ETLBox/) when working with ETLBox. It contains the default connectors and transformations. 
...
The connectors are in separate packages - depending on your needs, choose the right connector package from the list. Pick the [ETLBox.SqlServer](https://www.nuget.org/packages/ETLBox.SqlServer) package if you want to connect to a (Azure) SQLServer database, or choose [ETLBox.Json](https://www.nuget.org/packages/ETLBox.Json/") if you want to load data from a Json file or REST endpoint. 
...
Both components support the [logging API](https://docs.microsoft.com/en-us/dotnet/core/extensions/logging) that works with a variety of built-in and third party logging providers. 
...
- Internet access to retrieve order data from [www.etlbox.net/demo/api/orders](https://www.etlbox.net/demo/api/orders")

#### Preparation

Let's start with creating the destination table on the databases. If you have Docker installed, it is quite easy to run a [SQLServer](https://hub.docker.com/_/microsoft-mssql-server) in a container (you can also other any other supported database, e.g. [MySql](https://hub.docker.com/_/mysql)
...
[The whole code for this example is available on GitHub](https://github.com/etlbox/etlbox.demo/tree/main/QuickStart)
...
First step is to add a dependency to the latest nuget package [NLog.Extensions.Logging](https://www.nuget.org/packages/NLog.Extensions.Logging)
...
Now you are ready to try out ETLBox. All [ETLBox packages are available on nuget](https://www.nuget.org/packages?q=etlbox). The free versions allows you to process up to 5.000 for a component - this should be enough to perform tests in your test environment. If you are ready to deploy ETLBox in your production environment, [please purchase the adequate license](/buy/pricing/). If you want to try ETLBox without limitations, you can [request a free trial key](/support/contact/request-trial/).
